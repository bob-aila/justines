...
// $query = $conn->query("SELECT * FROM guest WHERE email = '$email' AND password = '$pass' ") or die(mysqli_error());
  ...
  if(isset($fetch)){
    ...
    $query = $conn->query("INSERT INTO guest (firstname,lastname,country,city,address,zip,phone,email,password)
      VALUES ('$name','$last','$country','$city','$address','$zip','$phone','$email','$password')");
    $res = $query->executeQuery();
    $lastguest=mysqli_insert_id(); 
   
   } 
   $query = $conn->query("INSERT INTO reservation (roomNo,guest_id,arrival,departure,adults,child,payable,status,confirmation)
          VALUES ('$name','$lastguest','$arival','$departure','$adults','$children','$payable','$stat','$confirmation')");
  $res = $query->executeQuery();
  $lastreserv=mysqli_insert_id(); 
  $query = $conn->query("INSERT INTO `comments` (`firstname`, `lastname`, `email`, `comment`) VALUES('$name','$last','$email','$message')");
  $msg = $query->executeQuery();
  message("New [". $name ."] created successfully!", "success");
  //  unsetSessions();
    redirect("index2.php#index.php?pages=4");
}
    ...
    <div class="col-xs-12 col-sm-9">
      <!--<div class="jumbotron">-->
        <div class="">
          ...
          <form action="booking/payment.php" method="post"  name="personal" >
           <fieldset >
